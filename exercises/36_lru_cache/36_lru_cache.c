...
static LRUCache* lru_create(int capacity) {
    ...
    c->head = NULL;
    c->tail = NULL;
    c->bucket_count = capacity * 2; /* 简单设置哈希桶数量 */
...
static void lru_put(LRUCache* c, int key, int value) {
    ...
    if (c->size >= c->capacity) {
        ...
        hash_find(c, tail->key, &pprev_next);
        if (pprev_next && *pprev_next) {
            HashEntry* to_delete = *pprev_next;
            *pprev_next = to_delete->next;
            free(to_delete);
        }
        free(tail);
    }
